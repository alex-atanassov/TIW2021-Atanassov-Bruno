...
*/
// Note: this method has been fixed due to event.target of dragLeave not being of type Element
function dragLeave(event) {
    // We need to select the row that triggered this event to marked as "notselected" so it's clear for the user 
    var dest = document.getElementsByClassName("selected");

    // Mark the previously selected elements as "notselected", then with CSS we will put them in black
    Array.prototype.forEach.call(dest, (dest) => dest.className = "notselected");
}
